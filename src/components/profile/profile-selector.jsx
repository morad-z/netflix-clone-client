...
export default function ProfileSelector({ onSelect, onAddProfile, isRedirecting }) {
  ...
  const updateProfileMutation = useMutation({
    mutationFn: async ({ id, name }) => {
      const response = await apiRequest("PUT", `/api/profiles/${id}`, {
        name,
      });
    ...
    <div className="flex flex-col items-center">
        ...
        {profiles.map((profile) => (
          <div key={profile.id} className="flex flex-col items-center">
            <div className="relative group">
              ...
              <div
                onClick={() => {
                  if (editingProfileId !== profile.id && !isRedirecting) {
                    onSelect(profile);
                  }
                }}
      ...
      {profiles.length > 0 && editingProfileId === null && (
        <Button
          ...
          onClick={() => {
            // Enable editing mode for all profiles
            setEditingProfileId('manage');
          }}
        >
          Manage Profiles
        </Button>
