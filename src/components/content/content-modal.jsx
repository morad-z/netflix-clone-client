...
export default function ContentModal({
  ...
  const { data: myListStatus, refetch: refetchMyList } = useQuery({
    queryKey: [
      `/api/mylist/${activeProfile?.activeProfile?.id}/check/${tmdbId}`,
    ],
    enabled: isOpen && !!tmdbId && !!activeProfile?.activeProfile?.id,
  });
  ...
  const addToMyListMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: `"${details?.title || details?.name}" has been added to your list`,
      });
    },
    onError: (error) => {
      toast({
  ...
  const removeFromMyListMutation = useMutation({
    mutationFn: async () => {
      ...
      await apiRequest("DELETE", `/api/mylist/${activeProfile?.activeProfile?.id}/${tmdbId}`);
    },
    onSuccess: () => {
      refetchMyList();
      // Also invalidate the /api/mylist query so the My List page updates immediately
      ...
      toast({
        ...
        description: `"${details?.title || details?.name}" has been removed from your list`,
      });
    },
    onError: (error) => {
      toast({
