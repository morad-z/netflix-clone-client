...
export default function TVShowsPage() {
  ...
  const { data: dramaShows } = useQuery({
    queryKey: ["/api/content/search", { genre: GENRE_IDS.DRAMA }],
    enabled: !!activeProfile,
    queryFn: async () => {
      const response = await fetch("/api/content/search?genre=" + GENRE_IDS.DRAMA, {
        credentials: 'include'
      });
      if (!response.ok) throw new Error("Failed to fetch drama shows");
      return await response.json();
    },
  });
  ...
  const { data: comedyShows } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!response.ok) throw new Error("Failed to fetch comedy shows");
      return await response.json();
    },
  });
  ...
  const { data: scifiShows } = useQuery({
    queryKey: ["/api/content/search", { genre: GENRE_IDS.TV_SCIFI_FANTASY }],
    enabled: !!activeProfile,
    queryFn: async () => {
      const response = await fetch("/api/content/search?genre=" + GENRE_IDS.TV_SCIFI_FANTASY, {
        credentials: 'include'
      });
      if (!response.ok) throw new Error("Failed to fetch sci-fi shows");
      return await response.json();
    },
  });
  ...
  const { data: crimeShows } = useQuery({
    queryKey: ["/api/content/search", { genre: GENRE_IDS.CRIME }],
    enabled: !!activeProfile,
    queryFn: async () => {
      const response = await fetch("/api/content/search?genre=" + GENRE_IDS.CRIME, {
        credentials: 'include'
      });
      if (!response.ok) throw new Error("Failed to fetch crime shows");
      return await response.json();
    },
  });
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <ContentModal
        ...
        type={selectedContent?.media_type || (selectedContent?.title ? "movie" : "tv")}
